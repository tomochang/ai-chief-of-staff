...
- 書き先: ルール的なもの → ルールファイル / 共有ルール、事実・ログ → メモリ配下

## 分析成果物→永続化ルール

- セッション中に**分析・レビュー・評価**を出力したら、**対象プロジェクトの `docs/` に保存する。** 会話に出力しただけで終わらせない。
- ファイル名: `<descriptive-name>-<YYYY-MM-DD>.md`（例: `v2-maturity-assessment-2026-02-11.md`）
- 保存先: プロジェクト固有のdocs配下、またはメモリ配下
- 保存後、即座に commit & push する
- **判断基準:** 500文字以上の構造化された分析出力は保存対象。短い回答や会話的なやり取りは対象外

## エージェント間議論→永続化ルール

- 他のAIエージェントと設計議論を行った場合、**議論の全体をストーリー形式でファイルに保存する。**
- 含める内容: 各ラウンドの指摘・反論・合意・解決策、生まれた設計成果物の一覧、次のアクション
- ファイル名: `<topic>-discussion-<YYYY-MM-DD>.md`（例: `design-review-discussion-2026-02-23.md`）
- 保存先: 対象プロジェクトの該当ディレクトリ
- **理由**: 議論のコンテキストはバッファに消えるため、後からAIや人間が参照できる形で永続化する必要がある
